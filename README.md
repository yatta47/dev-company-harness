...
- 次の一手は **`requires_human` の強制テスト1本 → 実走1周 → 実走で分かった壊れ方を回帰テストに固定 → Driver**
- 最後の Driver が一番重い。`Workflow（順序）× Harness（押し返す力）× Driver（回す力・止める力）`のうち、**Driver がほぼ 0**（`/work-run` のループは SKILL.md の散文で、機械は反復回数すら数えていない）
