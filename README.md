...
> **Status: 🚧 実装途上。**
> **状態機械・ゲート・CLI は動きます**（`dod → researching → planning → implementing → reviewing → packaging → approved` を end-to-end で通し、打ち切り経路 `abandoned` も含めて検証済み）。
> **まだ無いのはロール定義（`.claude/agents/`）と slash commands** です。つまり今は「人間が CLI を叩けば回る」状態で、エージェントに委譲する層がありません。
...
- 状態機械と CLI（`harness/core.py` / `harness/cli.py`）
  - `init` / `list` / `switch` / `status` / `validate` / `advance` / `return` / `verify` / `abandon` / `decision` / `event` / `metrics` / `doctor`
...
- **ロール定義（`.claude/agents/`）— 工程オーナー5 + advisor 3**
- **slash commands（`.claude/skills/`）**
- 秘書のプロファイル（`secretary/profile.example.md`）
- コスト停止条件（自律モードの従量課金対策）

つまり今は **人間が CLI を叩けば一周する**状態で、エージェントへ委譲する層が未着手です。
